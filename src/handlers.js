...
Handlers.prototype._fns = {
    ...
    'subscript-child-active_position': _descend(function (key, value, ref) {
        return key === ref
    }),
    ...
    'subscript-child-union': function (component, partial, contextManager) {
        ...
        component.expression.value.forEach(function (listable, index) {
            var _component = _.merge(listable, {
                operation: listable.operation ? listable.operation : 'subscript',
                scope: listable.scope ? listable.scope : 'child'
            });
    ...
    'subscript-descendant-union': function (component, partial, contextManager) {
        ...
        contextManager.commit({
            options: {
                relative: true,
                $pathRelativeToBranch: true
            }
        }, 'subscript-descendant-union', null, false); // merge options
//    var nodes = jp.nodes(partial.value, '..*', contextManager);
        ...
        nodes.forEach(function (node) {
            component.expression.value.forEach(function (listable, index) {
                var _component = _.merge(listable, {
                    operation: listable.operation ? listable.operation : 'subscript',
                    scope: listable.scope ? listable.scope : 'descendant'
                });
...
function eval_recurse(partial, src, operation, scope, handler, contextManager) {
    ...
    if (handler === undefined) {
        ...
        value = scripts.evalScript(partial, src, contextManager);
    } else if (handler === 'active') {
        value = scripts.evalActiveScript(partial, src, contextManager);
    } else {
