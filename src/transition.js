...
function transition(component, node, message) {
    ...
    const scope = component.scope.split('|')[0];
    const key = [component.operation, scope, component.expression.type].join('-');
    const link = TRANSITIONS[key];
    switch (link) {
        case '.@':
        case '.#':
        case '.@@':
        ...
        case '..[,]':
            return this.commit({$parent: node}, link); //noop
        default:
            throw new Error('Hit default case');
    }
}
