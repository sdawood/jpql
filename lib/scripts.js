...
function evalActiveScript(partial, component, contextManager) {
  ...
  console.log('map.script::', unescapeTagScript(map.script));
  var value = evaluate.bind(executionContext)(unescapeTagScript(map.script), executionContext);
  console.log('map.script::value::', value);
...
//    results = reduce.provider? evaluate(unescapeTagScript(reduce.script), executionContext) : value;
    console.log('map::provider::results', value);
  }
  return value;
}
