...
function evalActiveScript(partial, component, contextManager) {
  ...
  var value = evaluate(parseTazcript(map.script), executionContext);

  if(map.provider) { // && component.expression.active.reduce == {}
    results = reduce.provider? evaluate(parseTazcript(reduce.script), executionContext) : true;
    console.log('key::provider::results', results);
    return results;
  }
  function parseTazcript(script) {
