...
//    contextManager.commit({options: {applyToRoot: true, mark: false, leaf: false}}, 'pathNodes');
//    var nodes = traverseNodes(partial, 'subscript-descendant-union', contextManager);
    var nodes = jp.nodes(partial.value, '..*', contextManager);
    if (branchParent.options.applyToRoot) {
      nodes.unshift({ path: ['$'], value: partial.value});
