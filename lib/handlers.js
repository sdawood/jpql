...
Handlers.prototype._fns = {
  ...
  'subscript-descendant-filter_expression': function(component, partial, context) {
    ...
    var passable = function(key, value) {
      return evaluate(ast, { '@': value, $key: key });
    }
  ...
  'subscript-child-root': function(component, partial, context) {
    ...
    var util = require('util');
    console.log('subscript-child-root()', util.inspect(component, false, null), util.inspect(partial, false, null));
    if (component.branch) {
      results = handleBranch(component, results, context);
      var template = { expression: { type: 'union',
        scope: component.scope,
        operation: component.operation }
      }
      template.value = results.map(function(result) {
        return { expression: { type: 'identifier', value: result.value }};
      });
    }

    results = this._fns['subscript-child-identifier'](template)
    console.log('$.branch::', console.log(util.inspect(results, false, null)));
    return results;
  }
};
...
function evalScript(partial, script, context) {
  ...
  var ast = aesprim.parse(src).body[0].expression;
  var value = evaluate(ast, { '@': partial.value, '$': context.$root });
  return value;
}
...
function evalActiveScript(partial, script, context) {
  ...
  var ast = aesprim.parse(src).body[0].expression;
  var value = evaluate(ast, { '@': partial.value, $: context.$root });
  console.log('evalActiveScript', partial.vaue, context.$root, value)
  return value;
}
...
function handleBranch(component, partial, context) {
  var jp = require('..');
  var util = require('util');
  console.log('handleBranch()', 'component', util.inspect(component, false, null), 'partial', util.inspect(partial, false, null));
  var edgeResults = jp.pathNodes(partial, component.branch.path, {relative: true, branchPath: false, $root: context.$root, $parent: partial.value}); //relative nested path
  console.log('handleBranch()', 'edgeResults', util.inspect(edgeResults, false, null));
  return edgeResults;
}
