...
//    var nodes = jp.nodes(partial.value, '..*', contextManager);
    ...
    nodes.forEach(function(node) {
      component.expression.value.forEach(function(listable, index) {
        // SHOULD operation and scope be overwritten? (test)
  ...
  'subscript-child-root': function(component, partial, contextManager) {
    ...
    console.log('[$]', debug_inspect(nodeContext));
    if (component.branch) {
...
function evalScript(partial, script, contextManager) {
  ...
  console.log('evalScript::context::', executionContext);
...
function evalActiveScript(partial, script, contextManager) {
  ...
  var executionContext = scriptExecutionContext(partial, nodeContext);
  console.log('evalActiveScript::ctx::', executionContext);
  var value = evaluate(ast, executionContext);
  return value;
}
...
function handleBranchWith(component, partials, contextManager, branchContext, message) {
  ...
  contextManager.commit({parent$: branchContext.$parent, node$: branchContext.$node});
