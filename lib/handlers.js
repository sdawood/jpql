...
Handlers.prototype._fns = {
  ...
  'subscript-descendant-union': function(component, partial, contextManager) {
    ...
    contextManager.commit({options: {relative: true, $pathRelativeToBranch: true}}, 'subscript-descendant-union', null, false); // merge options
...
function eval_recurse(partial, component, operation, scope, handler, contextManager) {
  ...
  if (handler === undefined) {
    ...
    if(active.provider) { //event value_ready from script handler
      var nodeResult = {path: partial.path, value: key};
      if(active.map.provider) {
        if (active.map.tag) { // too deep, but we do it so that you won't have to! we trust the AST too.
          $tag = '$' + active.map.label;
          $$tag = contextManager.tag(active.map.tag, $tag, nodeResult);
          console.log('$$tag', JSON.stringify($$tag));
        }
        return [ nodeResult ]; // script has already produced nodes, must have been a mapper
      }
      return [ {path: partial.path + tag, value: key} ]; // script has already produced nodes, must have been a mapper
    }
