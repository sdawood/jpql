  ...
  'subscript-child-slice|active': function(component, partial) {
    if (is_array(partial.value)) {
      ...
      args = args.map(function(arg) {
        return _.isString(arg) ? evalActiveScript(partial, arg) : arg;
      });
...
//    return unique(results);
  ...
  'subscript-child-script_expression|active': scriptExpressionActive("subscript-child"),

  'member-child-script_expression|active': scriptExpressionActive("member-child"),

  'member-descendant-script_expression|active': scriptExpressionActive("subscript-descendant")
};
...
function scriptExpressionActive(operationScope) {
  return function (component, partial) {
    ...
    var exp = component.expression.value.slice(2, -2);
    return eval_recurse(partial, exp, _templates[operationScope]);
  }
}
