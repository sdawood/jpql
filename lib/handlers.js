...
Handlers.prototype._fns = {
  ...
  'member-child-identifier': function(component, partial, context) {
    ...
    if (value instanceof Object && key in value) {
      ...
      console.log('context$parent::', debug_inspect(context.$parent));
//      context.$parent = context.$node = results; // marking the exit-node before branching, a single result or a list of results would all be handled with the same context.$parent == context.$node
      if (component.branch) {
        context.$branch = context.$edge = context.$rel = partial;
  ...
  'subscript-child-slice': function(component, partial, context) {
    if (is_array(partial.value)) {
      ...
      console.log('context$parent::', debug_inspect(context.$parent));
//      context.$parent = context.$node = nodes; // marking the exit-node before branching, a single result or a list of results would all be handled with the same context.$parent == context.$node
      if (component.branch) {
        context.$branch = context.$edge = context.$rel = partial;
...
//      context.$parent = context.$node = nodes; // marking the exit-node before branching, a single result or a list of results would all be handled with the same context.$parent == context.$node
  ...
  'subscript-child-union': function(component, partial, context) {
    var results = [];
    context.$parent = partial; // union handlers are not allowed to modify $parent
    /** union members starting a branch and going through pathNodes are relative, single expressions should not call pathNodes
     *  // pushes a merged copy of context.options with the new object to the context stack, pathNodes pops the stack after finishing with the branch
     */
    var options = context.options({relative: true, $pathRelativeToBranch: true});
  ...
  'subscript-descendant-union': function(component, partial, context) {
    ...
    var self = this;
    var results = [];
    var nodes = traverseNodes(partial, 'subscript-descendant-union', context);
  ...
  'subscript-child-filter_expression': function(component, partial, context) {
    ...
    console.log('subscript-child-filter::context::', debug_inspect(context));
    var passable = function(key, value) {
      var _pass = evaluate(ast, {
        '@': value,
        '$key': key,
        '$leaf': value,
        '$branch' : context.$branch ? context.$branch.value : undefined, // '$edge' : context.$branch.value, '$rel' : context.$branch.value,
        '$parent': context.$parent.value, '$node': context.$node.value,
        '$': context.$root.value
      });
      console.log('subscript-child-filter::', _pass, key, "->", value, '$leaf::', value, '$parent::', context.$parent);
      return _pass;
    };
...
//    context.$parent = context.$node = results; // marking the exit-node before branching, a single result or a list of results would all be handled with the same context.$parent == context.$node
  ...
  'subscript-descendant-filter_expression': function(component, partial, context) {
    ...
    var src = component.expression.value.slice(2, -1);
    var ast = aesprim.parse(src).body[0].expression;
    console.log('subscript-descendant-filter::context::', debug_inspect(context));
    var passable = function(key, value) {
      var _pass = evaluate(ast, {
        '@': value,
        '$key': key,
      ...
      console.log('subscript-descendant-filter::', _pass, key, "->", value, '$leaf::', value, '$parent::', context.$parent);
      return _pass;
    }
...
function eval_recurse(partial, src, operation, scope, handler, context) {
  ...
  console.log('eval_recurse()::script context path::', debug_inspect(path));
...
function evalScript(partial, script, context) {
  console.log('evalScript::context::', debug_inspect(context));
  var src = script.slice(1, -1);
  var ast = aesprim.parse(src).body[0].expression;
  var value = evaluate(ast, {
    '@': partial.value,
    '$leaf': partial.value,
    '$branch' : context.$branch ? context.$branch.value : undefined,
    '$parent': context.$parent.value, '$node': context.$node.value,
    '$': context.$root.value
  });
  console.log('evalScript', 'script::', script, '$leaf::', partial.value, 'value::', value, '$parent::', context.$parent);
  return value;
}

function evalActiveScript(partial, script, context) {
  console.log('evalActiveScript::context::', debug_inspect(context));
  ...
  console.log('evalActiveScript', 'script::', script, '$leaf::', partial.value, 'value::', value, '$parent::', context.$parent);
  return value;
}
...
function traverseNodes(partial, operation, context) {
...
function handleBranch(component, partials, context) {
  ...
  context.$branch = context.$branch ? context.$branch : undefined; // undefined means trouble

  // only immediate parent == @ references moves forward along the branch traversal, $node and $branch stay untouched
  var edgeResults = jp.pathNodes(partials, component.branch.path, context); //relative nested path
