...
* Future Syntax Options
  *
  * Note the syntax for an active tagscript expression with map script only, #quoteAll builting is used in this example for demonestration
   *
   * (#quoteAll{$parent$.friends})
   *
   * It much resembles a function call, within parentheses
   *
   * (#quoteAll($parent$.friends))
   *
   * With reduce active script in the picture, the two syntax start to diverge
   * * ({$parent$.friends}):(#quoteAll{$args})
   * And assume we allow an #future option in the parser to use (()) for active scripts, with the gotcha of automatically switching-off support for sexy curly brackets in stead of squares to surround subscripts, at least with the current Lexer logic
   * The above becomes
   * * (#quoteAll($args)):(($parent$.friends))
   * Which makes me dizzy, specially if ((@@)) is prefered by the user over $args, all those @@ staring you down, but the user is King
   * on the other hand, with longer piping (#future) it looks more tolerable
   * (($parent$.friends)):(#quoteAll($args)):(#compress($args))
   * Now that ^^ or this
   * ({$parent$.friends}):(#quoteAll{$args}):(#compress{$args})
   * The second mingles well with angular expression syntax, with template {{}} and vertical bar | as a filter in stead of the : aggregation options in activeScripts and activeTagScripts
   * But the last word is for a nother time
   *
* */
