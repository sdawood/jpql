...
var MemorySubject = function(nodes, path, contextManager) {
  ...
  this.provideAs = function(target, nodes) {
    ...
    var mergeInto = function(acc, node) {
      acc[node.path.slice().pop()] = node.value;
      return acc;
    };
    console.log('nodes::', nodes);
    return {path: thisPath, value: nodes.reduce(mergeInto, target)};
  };
  ...
  this.template = function(alias, options) {
    ...
    *
    * */
    return ["({", thisPath,"})"].join(); // a taste of tomorrow
  }
