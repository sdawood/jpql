...
/*
*
* Interpreters can be capable walking an AST
*
* Interpreters can provide member, subscript, script, filter, etc ... only
* or can provide capabilities for aggregates
*
* [*]   = descend one level withing arrays, lists, aggregated Observables
* {*}   = grab all key, value properties of a node (Endpoint, PoJo, etc)
* ..*   = descend recursively (explore) starting from node
* ..[*] descend recursively starting from lists
* [n:m] slice is an aggregation of skip n, take m - n
*
* Interpreters should not be confused with unparsing, where we convert a nodes flatMapped result with {path, value} pairs into a pojo stub, a mock, an api stub, or an asynchrounous Observable or a Subject
*
* NodeSubject(s) For inspiration https://allthingsarchitectural.wordpress.com/2014/09/17/using-rxjs-subjects-as-a-proxy-between-two-angular-controllers
*/

/* Interpreter and ContextManager and ResourceNode refactoring spec for the new interpreter. For inspiration http://davidshariff.com/blog/what-is-the-execution-context-in-javascript/
*
* executionContextObj = {
 ...
 * Note identical role of:
 ContextManager[git flow].head() is carrying scoped tags down the shortest path === scopeChain, since all changes on curent branch are visible via contextManager.head()
 Data Scope Arguments [path, partial, key] +
 ResourceNode[$key, $parent, $branch, $node, $root] (Note how ResourceNode has a double role, since it is the same node retreiver from head() loaded with parent scope chain) and
  === variableObject
 ResourceNode === this
...
/*INDEA
  ...
  * as a candidate for evaluation-feature, a simple dry run that generates a scafolding object to hold the data
   ...
   * 2- in async mode, a POJO skeleton holding observable end points is created, a Taz client interface object, or TazShell from console or websockets TeactTazWidget or an active addressable TagService hosted in the cloud
   *
   *
   *
