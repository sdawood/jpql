...
var levelsReducer = function(nodes, target, source) {
  ...
  var createSubject = function (tag, values) {
    ...
    console.log('createSubject =>', require('util').inspect([subject], false, null));
    return [subject];
  };//Rx.Subject;
  var requires = function($tag, $$node, $$subject, $$isLeaf) {
     ...
     *
     * */
    return function (index, depth, source, defaultValue) {
      var $node, $subject;
      var $emptyNode = { path: undefined, value: undefined };
      var $emptySubject = {};
      $emptySubject[tag] = undefined;

      $node = $$node ? source[index] : $emptyNode;

      $subject = $$subject ? createSubject($tag, $node.value) : $emptySubject;

      // we can tell if node is leaf
      var $isLeaf = $$isLeaf ? depth === $node.path.length - 1 : undefined;
  ...
  */
  levels.forEach(function(level, depth) {
    ...
    if (depth == 0) {
      ...
      slidingParents = level.map(function(){ return target; });
      return;
    }
    level.forEach(function(key, index) {
      ...
      if (exists) {
        if (isLeaf ^ (true != parent[key])) { //leaf trying to overwrite an object, or object trying to overwrite a leaf at "key", in both cases we need a POJO
          ...
          console.log('upgradeDataStructure::', 'isLeaf::', isLeaf, depth, index, 'parent::', parent, 'key::', key, 'newnode::', newnode, 'newSubject::', newSubject);
          parent[key] = newnode;//reference to newSubject is stored in slidingSubjects below
        } else if (isLeaf) {
          console.log('CIRCULAR REFERENCE::STRUCTURE SHARING::Observable Merging:: existing and structurally superior!', depth, index, 'parent::', parent, 'key::', key, 'newnode::', newnode, 'newSubject::', newSubject);
          defaultValue = true;
          ...
          console.log('mergeInto::', 'isLeaf::', isLeaf, depth, index, 'parent::', parent, 'key::', key, 'newnode::', newnode, 'newSubject::', newSubject);
          parent[key] = newnode;
        ...
        console.log('NEW NODE::', 'isLeaf::', isLeaf, depth, index, 'parent::', parent, 'key::', key, 'newnode::', newnode, 'newSubject::', newSubject);
        parent[key] = newnode;
      }
