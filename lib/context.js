...
ContextManager.prototype.node = function (hunks, message, branch) { //sugar for bulk push and head
    ...
    var nodeList = (hunks instanceof Array) ? hunks : [hunks];//Array.prototype.slice.call(arguments, 1);
    nodeList.forEach(function(node) {
      ...
      this.merge(head, node)
      this._push(head, branch); // [node0, node1, {node1} + {opt}, ...]
    }.bind(this));
...
ContextManager.prototype.branch = function (hunks, message, name) { //sugar for bulk branch push and head (head alias is by convention used by branching code only)
    ...
    nodeList.forEach(function(hunk) {
      ...
      head.commit += 1;
      head.commitMessages.push(message || '');
      this.merge(head, hunk);
      this._push(head, name);
    }.bind(this));
...
ContextManager.prototype.commit = function(delta, message, branch) {
    var head = this.head(branch) // clone
    head.commit += 1;
    head.commitMessages.push(message || '');
    this._push(delta ? this.merge(head, delta) : head, branch);
    return this.head(branch); // return a clone
}
