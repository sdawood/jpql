...
ContextManager.prototype.initialize = function(rootOptions, meta) {
  console.log('initalize(argument)', arguments);
  this.commit({meta: meta});
  this.configure(rootOptions ? rootOptions : {applyToRoot: true, mark: false, leaf: false}); // @todo: read defaults from config;
}

ContextManager.prototype.configure = function(options) {
  this.commit({options: options}, 'configure(options)');
  return this;
}

ContextManager.prototype.transition = transition.transition;
ContextManager.prototype.types = require('./reactaz/node-types');
...
ContextManager.prototype.tag = function(family, label, node) {
  var tag = {label: '$' + label, family: family};
  tag[tag.label] = value; tag.path = node.path; tag[path] = label; tag.provider = tag['=>'] = node.path; tag.since = new Date();
  var nodeContext = this.head();
  nodeContext.tags = nodeContext.tags ? nodeContext.tags.concat([tag]) : [];
  contextManager.commit(nodeContext, JSON.stringify(tag));
  this._tags[tag.label] = value; //the caller code is not to attached to the internals of the tag, it lives inside the node context, and evidence is logged Context+Action+Result+Evidence
  return tag;
};
