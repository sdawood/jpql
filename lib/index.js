...
JSONPath.prototype.nodes = function(obj, string, contextManager) {
  ...
  if(contextManager === undefined ) {
    ...
    var rootNode = contextManager.node({$root: $root, $stem: $root, $parent: $root, $node: $root, $path: $root.path,
      options: {applyToRoot: true, mark: false, leaf: false}}, 'root');
  }
...
//        contextManager.node({$node: p}, '$node++['+partialsIndex+']' + p.path);
      ...
      console.log('parent to be::', this.inspect(results));
//      var $parent = results.map(function(result) { return result.value });
      var $parent = results.slice();
//      console.log('$parent::', this.inspect($parent));
//      $parent = $parent.length == 1 ? $parent.pop() : $parent;
      contextManager.commit({$parent: $parent}, '$parent['+partialsIndex+']:' + results.map(function(result){return result.path}));
    }.bind(this));
