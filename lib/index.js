...
JSONPath.prototype.initialize = function() {
  ...
  this.ContextManager = ContextManager; // new instance of ContextManager is created with every call to nodes(). Reference is passed around and never stored
};
...
JSONPath.prototype.configure = function (options, meta, contextManager) {
  this.contextManager = contextManager ? contextManager.configure(options, meta) :
    this.contextManager ? this.contextManager.configure(options, meta) :
      new this.ContextManager(options, meta);
  return this;
}

JSONPath.prototype.nodes = function(source, path, contextManager) {
  contextManager = contextManager || this.contextManager || new this.ContextManager(null, {location: '$'}); //legacy interpreter should have the contextManager injected and not instantiate one on its own discretion
  assert.ok(contextManager, 'Fatal: can not run without a ContextManager');
  assert.ok(source instanceof Object, "source needs to be an object");
  assert.ok(path, "path must be a string");
  var ast = this.parser.parse(path);
  var partials = [ { path: ['$'], value: source } ];
  var $root;
  if (ast.length && ast[0].expression.type == 'root') ast.shift();
  if (!path.length) return partials;
  var nodeContext = contextManager.head();
  this.reportResults = function (results) { return results; };
  if (nodeContext.meta.location === '$') {
    $root = [ { path: ['$'], value: source } ];
    nodeContext = contextManager.node({$root: $root, $stem: $root, $parent: $root, $node: $root, $path: $root.path}, '$', null, {location: '$'});
    console.log('nodeContext.options', nodeContext.options);
    if(nodeContext.options.activeTags){
      this.reportResults = function(results) {
        return {
          results: results,
          tags: contextManager.tags()
        };
      }
    }
  }
  var results = this.pathNodes(partials, ast, contextManager);
...
JSONPath.prototype.pathNodes = function(partials, path, contextManager) {
  ...
  path.forEach(function(component, index) {
    ...
    partialMatches.forEach(function(p, partialsIndex) {
      ...
      var $parent = results.slice();
      contextManager.commit({$parent: $parent}, '$parent[' + partialsIndex + ']:' + results.map(function(result) {return result.path.join('.')}));
    }.bind(this));
