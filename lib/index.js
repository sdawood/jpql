...
JSONPath.prototype.nodes = function(obj, string, contextManager) {
  ...
  if(contextManager === undefined ) {
    ...
    contextManager.node({$root: $root, $stem: $root, $parent: $root, $node: $root}, 'root');
    isRoot = true;
  }
  var results = this.pathNodes(partials, path, contextManager);
  console.log('results', this.inspect(results));
  if(isRoot) {
    console.log('commit history::', this.inspect(contextManager._nodes));
  }
  return results;
};
...
JSONPath.prototype.pathNodes = function(partials, path, contextManager) {
  ...
  path.forEach(function(component, index) {
    ...
    partials.forEach(function(p, partialsIndex) {
      var results = [];
      console.log('pathNodes()', 'index', index, 'of', path.length);
      if (options.relative) {
        var _parent = p; // immediate parent

        results = handler(component, { path: ['$'], value: _parent.value }, contextManager);
