...
JSONPath.prototype.configure = function (options, contextManager) {
  this.contextManager = contextManager || this.contextManager;
  if (options) this.contextManager.configure(options);
  return this;
}

JSONPath.prototype.nodes = function(source, path, contextManager) {
  contextManager = contextManager || new this.ContextManager(null, {location: '$'}); //legacy interpreter should have the contextManager injected and not instantiate one on its own discretion
  ...
  console.log(require('util').inspect(contextManager.head(), false, null));
  if (contextManager.head().meta.location === '$') {
    $root = [ { path: ['$'], value: source } ];
    contextManager.node({$root: $root, $stem: $root, $parent: $root, $node: $root, $path: $root.path}, '$');
  }
