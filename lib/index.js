...
JSONPath.prototype.nodes = function(obj, string, context) {
  ...
  if(!context.$root) context.$root = $root;
  if(!context.$parent) context.$parent = $root;
...
JSONPath.prototype.pathNodes = function(partials, path, context, options) {
  ...
  path.forEach(function(component, index) {
    ...
    partials.forEach(function(p) {
      ...
      if (options.relative) {
        var $parent = p.value;
        results = handler(component, { path: ['$'], value: p.value }, { $root: context.$root, $parent: $parent});
        results = results.map(function(result) {
          if (options.branchPath) result.branchPath = result.path;
          result.path = p.path.concat(result.path.slice(1));
          return result;
        });
      } else {
